...
- **发布脚本**: `/Users/jarvis/xiaohongshu-bot/xhs-option/publish.py`（使用 `/usr/bin/python3`）
- **Obsidian Vault**: `/Users/jarvis/xiaohongshu-bot/xhs-option/vault/`
  - 待发布文件夹: `待发布/`
  - 已发布文件夹: `已发布/`
...
**Method Selection:**
- **必须使用** `mcp__xhs-option__` 前缀工具（期权账号专用，端口 18060）
  - `mcp__xhs-option__publish_content` 发图文
  - `mcp__xhs-option__publish_with_video` 发视频
...
| 5. Publish | mcp__xhs-option__ | Published post URL |
...
```javascript
// 发布视频
mcp__xhs-option__publish_with_video({
  title: "期权波动率微笑曲线详解",
  content: "[完整正文内容]",
...
```bash
cd /Users/jarvis/xiaohongshu-bot/xhs-option && bash start_mcp.sh
```
...
3. 呈现草稿审核：
   ...
   **话题标签：** #期权教育 #Greeks #卖方思维
   ...
   ```
4. 用户：「✅通过」
5. 使用 mcp__xhs-option__ (port:18060) 发布
6. 确认：「发布成功！笔记链接：[URL]」
